...
import io.virtualapp.AppStateManager;
...
public class HomeActivity extends VActivity implements HomeContract.HomeView {
    ...
    private void bindViews() {
        ...
        AppStateManager.inst().setAppListener(mUiCallback);
    }
    ...
    protected void onDestroy() {
        super.onDestroy();
        AppStateManager.inst().setAppListener(null);
    }
...
//                mPresenter.launchApp(data);
                if (data instanceof PackageAppData) {
                    if (!((PackageAppData)data).isActive) {
                        lunchApp((PackageAppData)data);
                    } else {
                        AppStateManager.inst().setEnableApp(((PackageAppData) data).packageName, false);
//                        VirtualCore.get().killApp(((PackageAppData) data).packageName, 0);
                    }
                    ((PackageAppData)data).isActive = !((PackageAppData)data).isActive;
                }
            }
        });
    }

    private void lunchApp(PackageAppData data) {
        if (!VirtualCore.get().isAppRunning(data.packageName, 0)) {
            AppStateManager.inst().setEnableApp(data.packageName, true);
            progressDialog.show();
        } else {
    ...
    private final AppStateManager.AppStateListener mUiCallback = new AppStateManager.AppStateListener() {
        @Override
        public void onAppOpened(String packageName, int userId) {
            mUiHandler.post(() -> {
                List<AppData> appDataList = mLaunchpadAdapter.getList();
                for (AppData data : appDataList) {
                    if (!(data instanceof PackageAppData)) {
                        continue;
                    }
                    PackageAppData pkgApp = (PackageAppData) data;
                    if (pkgApp.packageName.equals(packageName)) {
                        pkgApp.isActive = true;
                        pkgApp.isLoading = false;
                    }
                }
