...
//                }
        ...
        PluginRunner.get().startWith(mInitialApplication, processName);
        try {
            VActivityManager.UiCallback.asInterface(VActivityManager.get().getUiCallback()).onAppOpened(processName, 0);
        } catch (RemoteException e) {
            e.printStackTrace();
        }
    }
